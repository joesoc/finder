...
import { ContentserviceService as ContentService} from '../../../../app/search/content/contentservice.service';
import { LoggerService } from 'src/app/common/logging/logger.service';
import { domRendererFactory3 } from '@angular/core/src/render3/interfaces/renderer';

@Component({
  selector: 'app-idoldatabasedropdown',
  templateUrl: './idoldatabasedropdown.component.html',
  styleUrls: ['./idoldatabasedropdown.component.css']
})
export class IdoldatabasedropdownComponent implements OnInit {
    // ux-select configuration properties
    options: string[] | Function;
    display: string = null;
    key: string = null;
    comboboxValue: string | string[];
    dropdownValue: string | string[];
    multiple = new BehaviorSubject<boolean>(true);
    allowNull = false;
    disabled = false;
    dropDirection = 'down';
    ...
    placeholder = 'Select an IDOL Database';
  ...
  constructor(private _contentsvc: ContentService, private _logger: LoggerService) {
           // Reset select when "multiple" checkbox changes.
           this.multiple.pipe(takeUntil(this._onDestroy)).subscribe(() => {
        ...
        this.pagingEnabled.pipe(takeUntil(this._onDestroy)).subscribe(() => {
            this.comboboxValue = null;
            this.dropdownValue = null;
            this.dropdownOpen = false;
            this.comboboxOpen = false;
        ...
        this._logger.log("Pushing DBs");
        this._contentsvc.getDatabases().subscribe(db => {
            this._logger.log("Push Dbs " + db);
            this.dataSets.strings.push(db);
        });
        // "objects" data set
